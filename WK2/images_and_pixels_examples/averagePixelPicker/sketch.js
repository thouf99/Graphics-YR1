...
function draw() {
  ...
  for (let row=0; row < boxSize; row++)
  {
    ...
    {
      // now sum up pixel by pixel to get total for r,g,b channels
      // what's the current pixel index in terms of mouseX,mouseY, row, column?

      ///// MODIFY THIS
      r += ??? // + current pixel r value
      b += ??? // + current pixel b value
      g += ??? //  + current pixel g value
    ...
    let avgR = r/boxSize*boxSize;
    let avgG = r/boxSize*boxSize;
    let avgB = r/boxSize*boxSize;

    // draw rectangle with average value
    fill(avgR, avgG, avgB);
